    ...
    while True:
        try:
            ...
            for file in ss_files:
                ...
                leaked_txs.append(decode_raw_tx(raw_transaction))
                print("read subversion service path",tx_filepath)
            if len(ss_files) > 0:
                break
        except FileNotFoundError as e :            
            print(f"err {e} reading subversion service path")
            time.sleep(5)
            pass
    new_bundle, r, sting_sender, stinger_hashes, unsigned_adv_tx = make_evidence(w3, leaked_txs)
