...
def approve_enclave(w3):
    ...
    enclaves = contract.functions.requested_enclaves(0).call({"from": bounty_admin.address})
    for addr in [enclaves]:
        sgx_data = contract.functions.requested_enclaves_data(addr).call({"from": bounty_admin.address})
        ...
        assert addr == enclave_address
        APPROVED_MRENCLAVE = mrenclave
        # assert mrenclave == APPROVED_MRENCLAVE #TODO reporoducible builds
        ...
        report_path = "enclave_report.check"
        sig_path = "ias_sig.check"
...
def deploy_contract(w3, admin_addr, contract_paths):
    ...
    tx_hash = contract.constructor().transact({"from": admin_addr, "value": 1000})
...
def refill_ether(w3, receiver_addr, amt):
    sender = Account.from_key(os.environ.get("ADMIN_PRIVATE_KEY","0xf380884ad465b73845ca785d7e125e4cc831a8267ed1be5da6299ea6094d177c"))
    print(f'sender {sender.address} balance: {w3.eth.get_balance(sender.address)}')
    if amt > 0:
        tx = {
            'to': receiver_addr,
            'from': sender.address,
        ...
        signed_tx = w3.eth.account.sign_transaction(tx, sender.privateKey)
        w3.eth.send_raw_transaction(signed_tx.rawTransaction)
        w3.eth.wait_for_transaction_receipt(signed_tx.hash)

    print(f'refilling {receiver_addr} updated: {w3.eth.get_balance(receiver_addr)}')
